using System;
using System.Collections.Generic;
using System.Linq;
using System.Text;
using System.Text.RegularExpressions;
using System.Security.Cryptography;

namespace BaseUtility
{
    public class Security
    {
        private static string publickey = @" <RSAKeyValue><Modulus></Modulus><Exponent>AQAB</Exponent></RSAKeyValue>";
        private static string privatekey = @"<RSAKeyValue><Modulus></Modulus><Exponent>AQAB</Exponent><P>9uZxAKKoqcXkpFamYDSmaFEggLvu3Ghw3rCsYpVzUjbIvYSvM1gsuG57jgOOl7xaoZ6XZR7froUKgv93doYw8G0Iq/AzcoW2a0/YHH7lCAo5dNZ79L3JfirX5OKz1+zNPm2iVefV3gqzkE5W3SvcHGrAeKIpC5uYa/mCUVOCR0k=</P><Q>3doYQuSOUGqT1Dm7Q1R8tk30FAN7e4xM3f0KlJ0aOZi9hWd4WtB+t5J+c5OKMrQEtohIMP93IYn8df5zog9kBOhUd2ML3VergxUxNi467mnf/3P6eqLSll4Ke/0u10lgNWAEWl1fZqMLDJJJD0XUZp9GuNVgt3WbsoJBa8l0Wn0=</Q><DP>wJBWTjKO7Wqkuu/B74LgzreHbCAnWcwzS9vrdzAss1B7HCTiKF968ZVp1Ac+LWYAIdF5Lqr3tjuZSTZKTCy4+qtvefprhcR31BZ/7SmjI7Qlv9SQ9P8Yqchsfd26eov8P9ZuKCYbDYS3K7ON2A4fLCKuXufKr3z5Ui1V0GwbTdE=</DP><DQ>xRG/C2zLnlOODa9a0Wxze92zOzP8tLrkbS0iAPGtIgy0DZTkLIeYiKjw+unMkU2oAhz9Q8kqofhY8tedEBBr4JAguAybtXrzN/XTSTW306DpAZqcneOU/U18MresrEDZ3Y/1TfAzlpCud/RaQd7d1msV/4pL56vpKqDaTE0ftmE=</DQ><InverseQ>GVtOfL0kHKkv10Z+2De+N6TWpegYBl+fSGysRUPvN68y0Cg9Hp/6yN7o9rfmO/AVamn6QVN5BTN0aIUFxOwf7AyuFFXHUvfWRN8n1yXZDuXanQUtnwXKwrEJjXiqM6u395/nRRk1z6jyuN3r+Tophk181Vxu7KMZqfSpiJPchB0=</InverseQ><D></D></RSAKeyValue>";

        /// <summary>
        /// SHA 512 암호화
        /// </summary>
        /// <param name="planText">암호화할 문자</param>
        /// <returns>암호화된 문자열</returns>
        public static string ShaEncryption(string planText)
        {
            UnicodeEncoding byteConverter = new UnicodeEncoding();
            byte[] dataToEncrypt = byteConverter.GetBytes(planText);
            byte[] encryptData = SHA512.Create().ComputeHash(dataToEncrypt);

            return Convert.ToBase64String(encryptData);
        }

        /// <summary>
        /// RSA 암호화
        /// </summary>
        /// <param name="planText">암호화할 문자</param>
        /// <returns>암호화된 문자열</returns>
        public static string RsaEncryption(string planText)
        {
            try
            {
                RSACryptoServiceProvider rsa = new RSACryptoServiceProvider();
                rsa.FromXmlString(publickey);

                UnicodeEncoding byteConverter = new UnicodeEncoding();
                byte[] dataToEncrypt = byteConverter.GetBytes(planText);
                byte[] encryptedData = rsa.Encrypt(dataToEncrypt, false);
                return Convert.ToBase64String(encryptedData);
            }
            catch (Exception ex)
            {
                CommonUtil.WriteLog(ex.Message);
                return "";
            }
        }

        /// <summary>
        /// RSA 복호화
        /// </summary>
        /// <param name="planText">복호화할 문자</param>
        /// <returns>복호화된 문자열</returns>
        public static string RsaDescryption(string planText)
        {
            try
            {
                RSACryptoServiceProvider rsaProvider = new RSACryptoServiceProvider();
                rsaProvider.FromXmlString(privatekey);
                byte[] buffer = Convert.FromBase64String(planText);
                byte[] plainBytes = rsaProvider.Decrypt(buffer, false);
                string plainText = Encoding.UTF8.GetString(plainBytes, 0, plainBytes.Length);
                return plainText.Replace("\0", "");
            }
            catch (Exception ex)
            {
                CommonUtil.WriteLog(ex.Message);
                return "";
            }
        }
    }
}
